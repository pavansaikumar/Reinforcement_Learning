...
class CrossEntropyQBrain:
    ...
    def __initialize_tensorflow(self):
        number_states  = self.num_lanes + (self.num_lanes * self.num_road_sections_in_q_values) ## input for the neural network
        number_action = self.num_actions

        tensorflow.reset_default_graph()## when ever there is a change in the number of rows then we have to do this

        self.car_road_tensor = tensorflow.placeholder(shape=[None, number_states], dtype=tensorflow.float32)
        ...
        ## Trying to see the sampling
        self.sample_actions_tensor = tensorflow.multinomial(logits= prediction_tensor, num_samples=1)
        ...
        cross_entropies_tensor = tensorflow.losses.softmax_cross_entropy(onehot_labels=tensorflow.one_hot(self.chosen_action_tensor, number_action),
    ...
    def on_after_move(self, action, crashed, num_advances, recent_road_states):
        ## Trying to see if we are making a good move or bad move
        ## if we make four steps success
...
# """
    def __update_qvalues(self, action, reward, recent_road_states):
        ...
        for current_state in learning_states:
            ...
            self.tensorflow_session.run(self.train_tensor, feed_dict={self.car_road_tensor: road_sections_and_car_position,
